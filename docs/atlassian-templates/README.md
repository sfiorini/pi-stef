...
| Jira Epic | [`jira-epic.md`](./jira-epic.md) | `jira_create_issue` / `jira_update_issue` (plain-text or ADF via `fields.description`) | A large, multi-sprint body of work. Captures goal & value, scope, out-of-scope, Epic-level acceptance criteria, rollout milestones, dependencies, and references; includes an execution-order ADF table derived from the Stories' `Blocks` links. |
