...
| --- | --- | --- |
| Artifact paths | `src/artifacts/paths.ts` | Canonical `ai_plan/<slug>/.fh-workflow` paths. |
...
```text
ai_plan/<slug>/
  .fh-team.lock/                  # transient lockdir while a run owns the folder
  .fh-workflow/
...
`runWorkflow` writes `.fh-workflow/workflow.json` after acquiring the folder lock. `ownerTool` defaults to `toolName`; wrappers may pass `ownerTool` when a parent workflow owns nested phases. `fh_team_auto` uses `ownerTool: "fh_team_auto"` while its nested plan and implement phases set `currentTool` to `fh_team_plan` and `fh_team_implement`. Each nested phase may mark metadata `completed` when it exits; the next phase reopens the same owner record and sets `status` back to `running`.

Normal handoff flows can opt in to a narrow owner claim with `allowOwnerTakeoverFrom`. `fh_team_implement slug=<plan>` uses this to claim a completed `fh_team_plan` folder for implementation while still rejecting auto/task/followup-owned folders.
...
await runWorkflow(
  {
    ...
    ownerTool: "fh_team_auto", // optional for nested workflows; omit when the invoked tool owns its own folder
    // allowOwnerTakeoverFrom: ["fh_team_plan"], // optional alternative for deliberate normal handoffs
    useWorktree: true,
    promptForResume,
    createReporter,
    resolveBaseline,
...
Keep this library generic. It must not import fh-team prompts, tmux, Telegram, plan parsers, or worktree helpers.
